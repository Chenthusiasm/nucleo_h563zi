...
/** Receive callback function. Only one instance is needed since there can be only one instance
 *  of the USB device CDC interface.
 */
static USBD_CDC_ReceiveCallback receiveCallback = NULL;
...
static int8_t USBD_CDC_Receive(uint8_t *Buf, uint32_t *Len)
{
	...
	if ((receiveCallback != NULL) && (Buf != NULL) && (Len != NULL))
	{
 ...
 * @return The USBD_StatusTypeDef return status as an uint8_t.
 */
 ...
 * @param callback  The receive callback function to register.
 */
