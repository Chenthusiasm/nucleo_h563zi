...
namespace AD408x {
    ...
    *     FifoWatermark) span two consecutive addresses, MSB at the higher address, and are read/written together in one
    *     SPI frame under the AD408x default interface settings (STRICT_REGISTER_ACCESS = 1, ADDR_ASCENSION = 0).
    ...
    */
    ...
    public:
        ...
        * off access; higher-order methods within this class that touch several registers as one logical sequence should
        * use ReadRaw() instead and call PrepareTransactionGroup() themselves exactly once for the whole sequence.
        *
        ...
        *        reset values.
        *
        * @note Calls PrepareTransactionGroup() once for this group of transactions. Empirically, without this the first
        * CFG-bus transaction after idle reads back 0x00 (and a write immediately followed by a readback also reads back
        * 0x00). ADI's own reference driver does the same thing ahead of a transaction group, which is where this idea
        * came from; root cause is unknown.
        *
    ...
    private:
        ...
        *
        * For use by higher-order methods within this class that call PrepareTransactionGroup() once, then touch several
        * registers as one logical group.
        *
        * @tparam Reg Register tag type (e.g. InterfaceConfigA). Must define Address and Fields.
        * @return The register's current content.
        */
